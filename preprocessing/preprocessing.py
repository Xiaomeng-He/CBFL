...
def mapping_case_id(df, 
    ...
    Create a dictionary that stores the one-to-one mapping of case_id and index, 
    then transform the original case ID to index.  
...
def create_time_features(df, 
    ...
    -------
    df: pandas.DataFrame
        Event log with two temporal features
    
    """
    # calculate time since the previous event in case
    # case id has already been mapped to index
    ...
    for i in range(1, len(df)): # start from 1 to avoid calculating i-1 when i=0
        if df[case_id].iloc[i] == df[case_id].iloc[i - 1]: # if it is not the first event in each case Note to self: using df[event_name].iloc[i] != 'SOC' is also fine
